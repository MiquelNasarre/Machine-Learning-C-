...
Image::Image(const char* fmt_filename, ...)
{
    ...
    __crt_va_start(ap, fmt_filename);
    if (_vsnprintf_s(filename, 512, _TRUNCATE, fmt_filename, ap) < 0) 
        throw "Could not create image from file";
    __crt_va_end(ap);
...
bool Image::save(const char* fmt_filename, ...) const
{
    ...
    __crt_va_start(ap, fmt_filename);
    if (_vsnprintf_s(filename, 512, _TRUNCATE, fmt_filename, ap) < 0) return false;
    __crt_va_end(ap);
...
bool Image::load(const char* fmt_filename, ...) 
{
    ...
    if (std::fseek(file, (long)bfOffBits, SEEK_SET) != 0) 
    { 
        fclose(file); 
        return false; 
    }
    ...
    {
        uint8_t* in = row;
        if (fread(in, sizeof(uint8_t), row_stride, file) != row_stride) { fclose(file); return false; }
